     ...
     for i, id in enumerate(ids_des):
          if id in ids_src:
               ...
               for count, corner in enumerate(corners_src[j][0]):
                    ...
                    feature_3D_des = depth_des[int(corners_des[i][0][count][1])][int(
...
def full_registration(path, max_correspondence_distance_coarse,
     ...
     for source_id in trange(int(n_pcds)):
          ...
          for target_id in range(source_id + 1, int(n_pcds), max(1, int(n_pcds/N_Neighbours))):
